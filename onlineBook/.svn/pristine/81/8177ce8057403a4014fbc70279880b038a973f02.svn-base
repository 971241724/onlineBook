...
public class MessageAction {
	...
	public String addMessage(Model model,HttpServletResponse response,String message_username,String name,String message_content,HttpSession session,Integer pc ) throws IOException{
		response.setContentType("text/html; charset=utf-8");
		PrintWriter out = response.getWriter();
		if(pc == null){
			pc = 1;
		}
		if(message_username.isEmpty() || message_username.length()<=0){
			out.flush();
		    out.println("<script>");
		    out.println("alert('您还没有登录，请先登录后再留言。');");
	...
	public String findbyallMessage(HttpSession session,Model model,Integer pc){
		if(pc == null){
			pc = 1;
		}
