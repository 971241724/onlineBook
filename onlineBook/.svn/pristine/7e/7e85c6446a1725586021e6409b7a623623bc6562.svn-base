...
<div class="pagination"><ul>
		     	...
		     	<li><a href="${pageContext.request.contextPath }/findUnDealOrder.action?pc=${1 }">首页</a></li>
		        <c:if test="${page.pc>1 }">
		           <li class="active">
			    		<a href="${pageContext.request.contextPath }/findUnDealOrder.action?pc=${page.pc-1 }">上一页</a>
			       </li>
			    </c:if>
			    <c:choose>
		    	...
		    	<c:forEach var="i" begin="${begin }" end="${end }">
			    	    <c:if test="${page.pc!=i }">
			    	        <li>
			    	    		<a href="${pageContext.request.contextPath }/findUnDealOrder.action?pc=${i }">${i }</a>
			    	        </li>
			    	    </c:if>
		    			<c:if test="${page.pc==i }">
			    ...
			    <c:if test="${page.pc<page.tp }">
			        <li>
			    		<a href="${pageContext.request.contextPath }/findUnDealOrder.action?pc=${page.pc+1 }">下一页</a>
			        </li>
			    </c:if>
			    <li>
			    	<a href="${pageContext.request.contextPath }/findUnDealOrder.action?pc=${page.tp }">尾页</a>
		        </li>
		     </ul>
...
<script>
	$(document).on('click','.bookList-btn',function(){
		if(window.confirm('您是否确认订单？')){
			...
			$.ajax({
				url: 'adminAddOrder.action',
				type: 'GET',
				dataType: 'html',
				data: {order_id:$tr.find('.order_id').html()},
				success:function(rq){
					location.href="findUnDealOrder.action";
				}
			});
		}
	});
